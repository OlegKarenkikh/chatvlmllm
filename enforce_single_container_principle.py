...
def get_all_vllm_containers():
    ...
    try:
        # ИСПРАВЛЕНИЕ: Ищем контейнеры по образу vllm/vllm-openai
        result = subprocess.run([
            "docker", "ps", "-a", "--filter", "ancestor=vllm/vllm-openai:latest", 
...
def remove_stopped_containers():
    ...
    try:
        ...
        result = subprocess.run([
            "docker", "ps", "-a", "--filter", "ancestor=vllm/vllm-openai:latest", "--filter", "status=exited",
            "--format", "{{.Names}}\t{{.ID}}"
        ], capture_output=True, text=True, timeout=10)
